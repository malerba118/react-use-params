...
var _initialiseProps = function _initialiseProps() {
  ...
  this.processParamBeforeStringify = function (schema, param) {
    return coerceBeforeStringify(schema, param);
  };

  this.parse = function (queryStr) {
    var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
        _ref$includeExcess = _ref.includeExcess,
        includeExcess = _ref$includeExcess === undefined ? false : _ref$includeExcess;

    var rawParams = queryString.parse(queryStr);
    var processedParams = {};
    _this.schemas.forEach(function (schema) {
      var key = schema.name;
      processedParams[key] = _this.processAfterParse(schema, rawParams[key]);
    });
    if (includeExcess) {
      return _extends({}, rawParams, processedParams);
    }
    return processedParams;
  };

  this.stringify = function (rawParams) {
    var _ref2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
        _ref2$includeExcess = _ref2.includeExcess,
        includeExcess = _ref2$includeExcess === undefined ? false : _ref2$includeExcess;

    var processedParams = {};
    _this.schemas.forEach(function (schema) {
      var key = schema.name;
      processedParams[key] = _this.processParamBeforeStringify(schema, rawParams[key]);
    });
    if (includeExcess) {
      return queryString.stringify(_extends({}, rawParams, processedParams));
    }
...
var createParamsHook = (function (history) {
  return function (paramsSchema) {
    ...
    useEffect(function () {
      syncQueryParams.current = debounce_1(function (value) {
        // Get all query params from url
        var allPrevParams = schema.parse(history.location.search, {
          includeExcess: true
        });

        var relevantPrevParams = schema.parse(history.location.search, {
          includeExcess: false
        });
...
export default createParamsHook;
